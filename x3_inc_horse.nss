...
{ // PURPOSE: Reset oRider to an unmounted appearance
    object oSoul =GetSoulStone(oRider);
    if(!GetIsObjectValid(oSoul))
      oSoul = oRider;

//    SendMessageToPC(oRider,"Rider = "+GetName(oSoul));

//    int nApp=GetSkinInt(oRider,"nX3_HorseRiderAppearance");
    int nApp=GetLocalInt(oSoul,"KU_STANDART_APPEARANCE");
...
//    int nTail=GetSkinInt(oRider,"nX3_HorseRiderTail");
    int nTail=GetLocalInt(oSoul,"SUBRACE_TAIL_TYPE");
...
//    SendMessageToPC(oRider,"Tail: "+IntToString(nTail)+"; Pheno: "+IntToString(nPheno)+"; Appearance: "+IntToString(nApp));
