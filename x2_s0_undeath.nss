...
// End of Spell Cast Hook
    ...
    int nCurHD;
    object oFirst = GetFirstObjectInShape(SHAPE_SPHERE, 20.0f, lLoc, TRUE );
     ...
     if  (GetIsObjectValid(oFirst))
     {
        ...
        while (GetIsObjectValid(oTarget) && nHDLeft >0)
        {
            ...
            // Get next target
            oTarget = GetNextObjectInShape(SHAPE_SPHERE, 20.0f, lLoc, TRUE);
            ...
            if (!GetIsObjectValid(oTarget))
            {
                ...
                if (GetIsObjectValid(oLow) && nHDLeft >= nLow)
                {
                    ...
                    nHDLeft -= nLow;
                    // restart the loop
                    oTarget = GetFirstObjectInShape(SHAPE_SPHERE, 20.0f, lLoc, TRUE);
                }
                // reset counters
                oLow = OBJECT_INVALID;
                nLow = 9999;
