...
*/
    ...
    else // tady predano melvikem jaarovo chrapani
    {
         // is night doplneno melvik -> nema cenu ani nacitat ci nastavovat promenne pres den, take pozice na zacatku rads ne, kontrolovat jen kdyz je pohoda
        int IsNight = GetIsNight();
        if(IsNight){
            int l = GetLocalInt(OBJECT_SELF, "JA_COMMONER_LIMBO");
            if(l==0)// kdyz neni promenna spat nastavena na 2
            {
                int iForce = GetLocalInt(OBJECT_SELF, "JA_NIGHT_LIMBO");
                if(GetIsCommoner(OBJECT_SELF) || iForce ){ //in night we'll disappear
                  if(!GetLocalInt(GetArea(OBJECT_SELF), "JA_NIGHT_NOLIMBO") || iForce){
                    if(Random(100) < 80 || iForce){    //80% - limbo
                        ...
                        ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eDis, OBJECT_SELF, HoursToSeconds(iDuration));
                        // melvik pro sychr kdyby dostavala dal heart beat -- google neodpovedel :)
                        SetLocalInt(OBJECT_SELF, "JA_COMMONER_LIMBO", 2);
                        DelayCommand(HoursToSeconds(iDuration+5), SetLocalInt(OBJECT_SELF, "JA_COMMONER_LIMBO", 0));
                        return;
                    }
                    else{                    //do not limbo
                        SetLocalInt(OBJECT_SELF, "JA_COMMONER_LIMBO", 2);
