...
{
    return GetLocalInt(oPC, sNeedsEveryVar + IntToString(nItemIdx));
}
...
void SetPCNeedsEvery(object oPC, int nItemIdx, int nHours)
{
    ...
    SetLocalInt(oPC, sNeedsEveryVar + IntToString(nItemIdx), nHours);
    return;
}
...
{
    return GetLocalInt(oPC, sAddictedVar + IntToString(nItemIdx));
}
...
{
    SetLocalInt(oPC, sAddictedVar + IntToString(nItemIdx), nState);
    return;
}
...
int GetIsPCInWithdrawal(object oPC, int nItemIdx)
{
    ...
    if (GetIsPCAddicted(oPC, nItemIdx) == TRUE)
    {
        nInWithdrawal = GetLocalInt(oPC, sWithdrawalVar + IntToString(nItemIdx));
...
{
    SetLocalInt(oPC, sWithdrawalVar + IntToString(nItemIdx), nState);
    return;
}
...
{
    SetLocalInt(oPC, sNeedHourVar + IntToString(nItemIdx), nHour);
    return;
}
...
{
    return GetLocalInt(oPC, sNeedHourVar + IntToString(nItemIdx));
}
...
{
    SetLocalInt(oPC, sLastUsedHourVar + IntToString(nItemIdx), GetTimeHour());
    return;
}
...
{
    return GetLocalInt(oPC, sLastUsedHourVar + IntToString(nItemIdx));
}
...
{
    SetLocalInt(oPC, sUseCountVar + IntToString(nItemIdx), nCount);
    return;
}
...
{
    return GetLocalInt(oPC, sUseCountVar + IntToString(nItemIdx));
}
