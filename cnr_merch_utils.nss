...
int CnrMerchantDestroyItemOnObject(string sItemTag, object oTarget, int nItemCount)
{
    ...
    while (GetIsObjectValid(oItem))
    {
      if (GetTag(oItem) == sItemTag)
      {
        ...
        nItemCount = nItemCount + nStackSize;
        DestroyObject(oItem);
      }
      oItem = GetNextItemInInventory(oTarget);
    }
...
*/
int CnrHasItem(string sItemTag, object oTarget)
{
...
int CnrMerchantShowAsRed(object oMerchant, int nMenuIndex)
{
  ...
  if (sMenuType == "BUY")
  {
    // see if PC speaker has items with this tag
    string sItemTag = CnrMerchantGetBuyTag(sKeyToCurrentMenu, nItemIndex);
    if (CnrHasItem(sItemTag, oPC))
    {
      return FALSE;
    }
    else
...
void CnrMerchantBuyOrSellItem(object oMerchant, int nMenuIndex)
{
  ...
  if (sMenuType == "BUY")
  {
    ...
    int nSetSize = CnrMerchantDestroyItemOnObject(sBuyTag, oPC, 0);
    // Remove the appropriate qty of items from the PC and
    nSetSize = CnrMerchantDestroyItemOnObject(sBuyTag, oPC, nSetSize);
    GiveGoldToCreature(oPC, nBuyPrice*nSetSize);
