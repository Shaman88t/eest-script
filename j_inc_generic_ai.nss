...
{
    // Feat
    //if(GetTag(OBJECT_SELF) == "Caine") SpeakString("I'm hereS.");
    if(iRequirement == iM1)
    {
        if(GetHasFeat(iThingID))
        {
            //if(GetTag(OBJECT_SELF) == "Caine") SpeakString("I'm here1. Feat:"+IntToString(iThingID));
...
int AI_ActionUseEpicSpell(int nFeat, int nSpell, object oTarget = OBJECT_SELF)
{
    if(GetHasFeat(nFeat))
    {
        ...
        //SpeakString(IntToString(nFeat)+":"+IntToString(nSpell));
        // We turn off hiding/searching
        AI_ActionTurnOffHiding();
...
//::99999999999999999999999999999999999999999999999999999999999999999999999999*/
    ...
    if(IsFirstRunThrough && !GlobalInTimeStop && GlobalIntelligence >= i10 &&
       GetHasSpell(SPELL_TIME_STOP) >= i1)        //edited by Jaara //GetHasSpell(SPELL_TIME_STOP) >= i2
    {
 ...
 298 | Pulse_Disease - Varying Save. See diseases.2da, and it is based on the
    ...
    // - Checks if we have the effects or not in a second
      (/*(GlobalMeleeAttackers > (GlobalOurHitDice / i4) ||*/
       (GlobalMeleeAttackers > i0 && d10() > i6)/*)*/) && //edited by Jaara on 20.4.2006
...
//    if(GetTag(OBJECT_SELF) == "Caine") SpeakString("I'm here5. TARGET: ");
...
//    GlobalNearestEnemyHeard = GetNearestCreature(CREATURE_TYPE_REPUTATION, REPUTATION_TYPE_ENEMY, OBJECT_SELF, i1, CREATURE_TYPE_PERCEPTION, PERCEPTION_HEARD, CREATURE_TYPE_IS_ALIVE, TRUE);
    GlobalNearestEnemySeen = GetNearestCreature(CREATURE_TYPE_REPUTATION, REPUTATION_TYPE_ENEMY, OBJECT_SELF, i1, CREATURE_TYPE_PERCEPTION, PERCEPTION_SEEN);   //fixed by jaara
    GlobalNearestEnemyHeard = GetNearestCreature(CREATURE_TYPE_REPUTATION, REPUTATION_TYPE_ENEMY, OBJECT_SELF, i1, CREATURE_TYPE_PERCEPTION, PERCEPTION_HEARD);
    // Valids?
