...
//     oCont = Persist_GetTempContainer();
  ...
  string sSql = "SELECT item vars FROM pw_persist_items WHERE id = "+IntToString(id)+";";
//  SpeakString(sSql);
  SQLExecDirect(sSql);
  if (SQLFetch() == SQL_SUCCESS) {
//    string sID = IntToString(id);
    string sItem = SQLGetData(2);
    object oItem = Persist_CreateItemFromAttributesString(sItem,oCont);
...
void Persist_ReCreateItemsFromDB(object oCont) {
 ...
 /* ~ end hack */
  ...
  string sSql = "SELECT id, item FROM pw_persist_items WHERE container = "+sContID+";";
//  SpeakString(sSql);
  SQLExecDirect(sSql);
  string sAttr = "";
  string sID;
...
/*  if(GetLocalInt(oCont,"KU_PERSISTANT_BLOCKED")) {
  ...
  string sSql = "SELECT id, item FROM pw_persist_items WHERE container = "+sContID+" AND id >"+IntToString(From)+" ORDER BY id;";
//  SpeakString(sSql);
  SQLExecDirect(sSql);
  string sAttr = "";
  string sID;
...
int Persist_SaveItemToDB(object oItem, string sContID="") {
 ...
 string sItemSerialize =  NWNX_Object_Serialize(oItem);
...
// string sSQL = "INSERT INTO pw_persist_items  (container,spec,attributes,descr) VALUES ("+sValues+"); SELECT LAST_INSERT_ID();";
 string sSQL = "INSERT INTO pw_persist_items  (container,item) VALUES ("+sValues+");";
 SQLExecDirect(sSQL);
...
void Persist_ShopMoveCreatedItems(object oShop, object oChest, int bEmpty=0) {
  ...
  while(GetIsObjectValid(oItem)) {
    ...
    NWNX_Item_SetBaseGoldPieceValue(oNew,GetLocalInt(oItem,"GOLDPIECEVALUE"));
...
int Persist_ShopDisturbedItem(object oItem, object oPC, object oFrom) {
  ...
  if(GetLocalInt(oItem,"ORIG_PRICE")) {
    NWNX_Item_SetBaseGoldPieceValue(oItem,GetLocalInt(oItem,"ORIG_PRICE"));
  }
...
//  SetGoldPieceValue(oShopItem,GetLocalInt(oNewItem,"GOLDPIECEVALUE"));
  NWNX_Item_SetBaseGoldPieceValue(oShopItem,iPrice);
      ...
      case 1:
        ...
        if(iObject_type == OBJECT_TYPE_CREATURE) {
          ...
          if(GetStringLength(sAppearance) > 0) {
            ...
            oNPC = oPlc;
            NWNX_Creature_JumpToLimbo(oNPC);
          ...
          NWNX_Object_SetAppearance(oPlc,StringToInt(sAppear));
        }
        SetLocalInt(oPlc,"ku_plc_origin",2); /* Created from persistance */
