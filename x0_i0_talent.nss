...
talent GetCreatureTalent(int nCategory, int nCRMax, int bRandom=FALSE, object oCreature = OBJECT_SELF)
{
    ...
    if (bRandom == FALSE)
    {
        //SpeakString("Creature Talent");
        tTal = GetCreatureTalentBest(nCategory, nCRMax, oCreature);
        int iSpellId = GetIdFromTalent(tTal);
        if( iSpellId == SPELL_SHADOW_CONJURATION_SUMMON_SHADOW ||
    ...
    // * randomize it
    {
        //SpeakString("Creature Talent Random");
        tTal = GetCreatureTalentRandom(nCategory, oCreature);
        int iSpellId = GetIdFromTalent(tTal);
        if( iSpellId == SPELL_SHADOW_CONJURATION_SUMMON_SHADOW ||
            iSpellId == SPELL_GREATER_SHADOW_CONJURATION_SUMMON_SHADOW){
         if( !GetIsObjectValid(GetAssociate(ASSOCIATE_TYPE_SUMMONED)) ){
...
int TalentSpellAttack(object oIntruder)
{
    ...
    if(!GetIsObjectValid(oTarget) || GetArea(oTarget) != GetArea(OBJECT_SELF) || GetPlotFlag(OBJECT_SELF) == TRUE)
    {
        ...
        {
            oTarget = GetNearestSeenEnemy();
            //SpeakString("Get Nearest Seen or Heard: " + ObjectToString(oTarget));
    ...
    if (GetIsTalentValid(tUse) == FALSE)
    {
        ...
        if (GetIsTalentValid(tUse) == FALSE)
        {
            //SpeakString("SpellAttack: INDiscriminant AOE not valid");
    ...
    if(GetIsTalentValid(tUse))
    {
        if( (GetTypeFromTalent(tUse) == TALENT_TYPE_SPELL
             && !GetHasSpellEffect(GetIdFromTalent(tUse), oTarget)) )
        {
            //SpeakString("Valid talent found AREA OF EFFECT DISCRIMINANT");
    ...
    if(bValid == FALSE)
    {
        ...
        {     //  SpawnScriptDebugger();
            ...
            if (genericAttemptHarmfulRanged(tUse, oTarget) == TRUE)
            {
                //SpeakString("Spell Attack Harmful Ranged Chosen");
    ...
    if (bValid == TRUE)
    {
        ...
        // DebugPrintTalentID(tUse);
        //SpeakString("Talent Spell Attack Successful Exit");
    ...
    */
    //SpawnScriptDebugger();
     ...
     {
      tUse = GetCreatureTalentRandom(TALENT_CATEGORY_HARMFUL_RANGED);
      //SpeakString("TALENT_CATEGORY_HARMFUL_RANGED");
     }
     if (GetIsTalentValid(tUse) == FALSE || bkTalentFilter(tUse, oTarget, TRUE)==FALSE)
     {
      tUse = GetCreatureTalentRandom(TALENT_CATEGORY_HARMFUL_AREAEFFECT_INDISCRIMINANT);
      //SpeakString("TALENT_CATEGORY_HARMFUL_AREAEFFECT_INDISCRIMINANT");
     }
     if (GetIsTalentValid(tUse) == FALSE || bkTalentFilter(tUse, oTarget, TRUE)==FALSE)
     {
      tUse = GetCreatureTalentRandom(TALENT_CATEGORY_HARMFUL_TOUCH);
      //SpeakString("TALENT_CATEGORY_HARMFUL_TOUCH");
     }
...
int TalentSeeInvisible()
{
    ...
    int nSpell;
    int bValid = FALSE;
    if( GetHasSpellEffect(SPELL_TRUE_SEEING) |
        GetHasSpellEffect(SPELL_INVISIBILITY_PURGE) |
        GetHasSpellEffect(SPELL_SEE_INVISIBILITY) |
