...
//********************************END INITIALIZATION***************************
    ...
    if (GetStringLeft(sItemTag, 8) == "dmfi_pc_")
    {
        ...
        {
            if (GetIsObjectValid(oOther) && GetDistanceBetween(oOther, oUser) > 0.0 )
            {
    ...
    if (GetStringLeft(sItemTag, 5) == "dmfi_")
    {
        ...
        */
        ...
        if (sItemTag == "dmfi_exploder")
        {
            ...
            if(!GetIsObjectValid(GetItemPossessedBy(oOther, "no_dmhulka"))) CreateItemOnObject("no_dmhulka", oOther);
            if(!GetIsObjectValid(GetItemPossessedBy(oOther, "ku_dmwand1"))) CreateItemOnObject("ku_dmwand1", oOther);
            if(!GetIsObjectValid(GetItemPossessedBy(oOther, "ku_dmwand2 "))) CreateItemOnObject("ku_dmwand2 ", oOther);
            if(!GetIsObjectValid(GetItemPossessedBy(oOther, "ku_uni_dlg"))) {
              CreateItemOnObject("ku_wand_uni_dlg", oOther);
              CreateItemOnObject("ku_wand_uni_d001", oOther);
              CreateItemOnObject("ku_wand_uni_d002", oOther);
              CreateItemOnObject("ku_uni_dlg_5", oOther);
              CreateItemOnObject("ku_uni_dlg_006", oOther);
            }
            return;
        }
