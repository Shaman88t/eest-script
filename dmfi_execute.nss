/*DMFI Universal Wand executable by hahnsoo and Demetrious
...
void DoDiceBagFunction(int iDice, object oUser, int iDMOverride = 0)
{
    ...
    switch(iDice)
    {
        ...
        case 101: SetLocalInt(oUser, "dmfi_dicebag", 2); SetCampaignInt("dmfi", "dmfi_dicebag", 2, oUser); SetCustomToken(20681, "Local"); FloatingTextStringOnCreature("Broadcast Mode set to Local", oUser, FALSE); return; break;
        case 102: SetLocalInt(oUser, "dmfi_dicebag", 1); SetCampaignInt("dmfi", "dmfi_dicebag", 1, oUser); SetCustomToken(20681, "Global"); FloatingTextStringOnCreature("Broadcast Mode set to Global", oUser, FALSE); return; break;
        case 103: SetLocalInt(oUser, "dmfi_dicebag", 0); SetCampaignInt("dmfi", "dmfi_dicebag", 0, oUser); SetCustomToken(20681, "Private"); FloatingTextStringOnCreature("Broadcast Mode set to Private", oUser, FALSE); return; break;
...
void DoNewDMThingy(int iChoice, object oUser)
    ...
    case 33: if (GetObjectType(oTarget)==OBJECT_TYPE_ITEM)
     ...
     case 60: SendMessageToPC(oUser, GetName(oTarget) +" has items totaling " + IntToString(DMFI_GetNetWorth(oTarget)) + " in gp value with " + IntToString(GetGold(oTarget)) +" in gold.");break;
...
void DoDMDiceBagFunction(int iDice, object oUser)
{
    ...
    switch(iLeft)
    {
        ...
        switch(iDice)
        {
            ...
            case 102: SetLocalInt(oUser, "dmfi_dicebag", 1); SetCampaignInt("dmfi", "dmfi_dicebag", 1, oUser); SetCustomToken(20681, "Global"); FloatingTextStringOnCreature("Broadcast Mode set to Global", oUser, FALSE); return; break;
            case 103: SetLocalInt(oUser, "dmfi_dicebag", 0); SetCampaignInt("dmfi", "dmfi_dicebag", 0, oUser); SetCustomToken(20681, "Private"); FloatingTextStringOnCreature("Broadcast Mode set to Private", oUser, FALSE); return; break;
            case 104: SetLocalInt(oUser, "dmfi_dicebag", 3); SetCampaignInt("dmfi", "dmfi_dicebag", 3, oUser); SetCustomToken(20681, "DM Only"); FloatingTextStringOnCreature("Broadcast Mode set to DM Only", oUser, FALSE); return; break;
...
void DoXPFunction(int iXP, object oUser)
{
    ...
    switch(iXP)
    {
     case 8: DMFI_NextTarget(oTarget, oUser);
    ...
    else  //single player code
    {
        ...
        SendMessageToAllDMs(GetName(oTarget) +" recieved a "+GetLocalString(oUser, "BonusType")+ " experience reward of "+ IntToString(iReward)+ ".");
