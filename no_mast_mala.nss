...
if (GetLocalInt(no_oPC,"no_mast_boj") == FALSE)
                      { effect no_lecim = EffectVisualEffect(IP_CONST_CASTSPELL_CURE_MINOR_WOUNDS_1,FALSE);
                        ...
                        ApplyEffectToObject(DURATION_TYPE_INSTANT,EffectHeal(4),no_oPC);
                        SetLocalInt(no_oPC,"no_mast_leceni",TRUE);
                        //ted  jeste blidness at nic kolem nevidis
...
void main()
{
if (GetResRef(GetItemActivated())=="no_mast_mala") {
...
if  ( GetLocalInt(no_oPC,"no_mast_leceni") == TRUE ) {
     ...
     effect no_dazed = EffectVisualEffect(IP_CONST_CASTSPELL_DAZE_1,FALSE);
                        ApplyEffectToObject(DURATION_TYPE_TEMPORARY,no_dazed,no_oPC,10.0);
                        ApplyEffectToObject(DURATION_TYPE_TEMPORARY,EffectDazed(),no_oPC,10.0);
     }
...
AssignCommand(no_oPC, ActionPlayAnimation(ANIMATION_LOOPING_SIT_CROSS, 1.0,60.0));
...
DelayCommand(60.1,SetLocalInt(no_oPC,"no_mast_leceni",FALSE));
DelayCommand(60.1,SetLocalInt(no_oPC,"no_mast_boj",FALSE));
