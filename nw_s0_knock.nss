...
// End of Spell Cast Hook
    ...
    oTarget = GetFirstObjectInShape(SHAPE_SPHERE, 50.0, GetLocation(oCaster), FALSE, OBJECT_TYPE_DOOR | OBJECT_TYPE_PLACEABLE);
    float fDelay;
    int nResist;
    ...
    while(GetIsObjectValid(oTarget))
    {
        ...
        if(!GetPlotFlag(oTarget) && GetLocked(oTarget))
        {
            ...
            if (nResist == 0 && ((GetCasterLevel(oCaster) + d20(1) + 5) >= GetLockUnlockDC(oTarget) ))
            {
...
/*            else if  (nResist == 1)
        ...
        oTarget = GetNextObjectInShape(SHAPE_SPHERE, 50.0, GetLocation(oCaster), FALSE, OBJECT_TYPE_DOOR | OBJECT_TYPE_PLACEABLE);
    }
}
