...
//Sets as enemy for creatures in range
void SetIsTemporaryEnemyInSphere( object oTarget, int bLineOfSight=TRUE, float fRange=20.0f, int bDecays=TRUE, float fDurationInSeconds=300.0){
    object oEnemy = GetFirstObjectInShape(SHAPE_SPHERE, fRange, GetLocation(oTarget), bLineOfSight, OBJECT_TYPE_CREATURE);
    //Cycle through the targets within the spell shape until an invalid object is captured.
    while (GetIsObjectValid(oEnemy))
    {
       SetIsTemporaryEnemy(oTarget, oEnemy, bDecays, fDurationInSeconds);
       AssignCommand(oEnemy, DetermineCombatRound(oTarget));

       oEnemy = GetNextObjectInShape(SHAPE_SPHERE, fRange, GetLocation(oTarget), bLineOfSight, OBJECT_TYPE_CREATURE);
    }
}
...
void SavePlayer(object oPC, int bSaveSpellsAndFeats=1){
    if(!GetIsPC(oPC) || GetIsDMPossessed(oPC)) return;
     ...
     if(GetTag(GetAreaFromLocation(lLoc)) != "VitejtevThalii")
