...
// End of Spell Cast Hook
   ...
   effect eFinal = EffectLinkEffects(eParalyze, eImmobile);
   eFinal = EffectLinkEffects(eFinal, eStop);
   ...
   SetLocalInt(OBJECT_SELF, "AI_JAARA_TIMESTOP", 1);
   DelayCommand(fDuration, DeleteLocalInt(OBJECT_SELF, "AI_JAARA_TIMESTOP"));
   ...
   while (GetIsObjectValid(oCreature)) {
       if (oCreature != oCaster) {
           ...
           DelayCommand(0.75, ApplyEffectToObject(DURATION_TYPE_TEMPORARY, eFinal, oCreature, fDuration));
       }
       oCreature =  GetNextObjectInShape(SHAPE_SPHERE, fSPELL_TIMESTOP_RADIUS, lTarget, FALSE, OBJECT_TYPE_CREATURE);
   }

   SetLocalInt(oCaster,"ku_spell_timestop_dur",ku_GetTimeStamp(FloatToInt(fDuration+0.75)));
