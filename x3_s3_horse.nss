...
//   SendMessageToPC(oPC,"Executed script x3_s3_horse");

   if (GetIsEnemy(oTarget))
     return; // no assigning enemies

   // No mount in interior and underground
   if ( ( GetIsAreaInterior(oAreaTarget) ) || ( !GetIsAreaAboveGround(oAreaTarget) ) )
         bNoMounts=TRUE;
       ...
       { // Dismount Menu Selection
           ...
           { // Creature
               ...
               { // mounted
                   ...
                   { // valid target
                       if (!GetIsPC(oTarget))
                         AssignCommand(oTarget,HORSE_SupportDismountWrapper(TRUE,TRUE));
                       else {
                         oHorse = HorseDismount(TRUE,TRUE);
                       }
                   } // valid target
                   else
