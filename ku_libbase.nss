...
/*
 * Funkce pro nacteni ECL Class
 * Returns ECLClass
 */
int ku_GetECLClass(object oPC)
{
/* // Global object to store the values on.
 object oModule = GetModule();
 // The base field name to store the values under.
 int a_nSubrace = GetLocalInt( oPC, SUBRACE_FIELD );
// int a_nSubrace = SEI_SubraceIDToIdx( GetLocalInt( oPC, SUBRACE_FIELD ) );
 string sTag = SUBRACE_STRUCT_TAG + IntToString(a_nSubrace) + "_";
 int ECL = GetLocalInt( oModule, sTag + "ECLC" );

// if(KU_XPPT_DEBUG)
//   SendMessageToPC(oPC,"XP_DEBUG ECL=" + IntToString(ECL));
*/
 return SEI_GetECLAdd( oPC );
}
...
//  int HitDice = GetHitDice(oPC) - 10; //redukce XP
  int HitDice =  FloatToInt(0.5 + sqrt(0.25 + ( IntToFloat(GetXP(oPC)) / 500 ))) - 10;
