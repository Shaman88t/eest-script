...
no_posta = GetLocalString(OBJECT_SELF,"no_posta");
    if (no_kamjde == no_posta)  {  //tak uz vime, ze miri sem
                                    ...
                                    DelayCommand(no_delay,SpeakString(" *prohlidne si balik* Tenhle balik uz tu davno mel byt. Dostanes za nej jen desetinu ceny, coz dela " + IntToString(no_cena/5) + " zlataku"));
...
if (no_pocet_provedeni >1) {
  DelayCommand(no_delay,SpeakString(" Takze dohromady za tyhle "+ IntToString(no_pocet_provedeni) + " baliky  je to " + IntToString(no_price) + " zlataku " ));
  }
