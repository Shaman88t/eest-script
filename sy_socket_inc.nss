...
// Do NOT set this higher than 20
const int MAX_VAMP_REGEN_BONUS = 3;

// Maximum Massive Critical allowed on a weapon
// Do NOT set this higher than 20
const int MAX_MASS_CRITS = 12;
...
void SF_DecrementSocket(object oPC, object oItem, object oTarget, int iSockets, int iBonus)
{
    if ((!STACKABLE_GEMS_SHARE_SLOT  && iBonus > 0) || !iBonus)
    {
        if (iSockets > 0)
        {
            ...
            if (iSockets == 0){
                IPRemoveMatchingItemProperties( oItem, IP_CONST_CASTSPELL_UNIQUE_POWER, DURATION_TYPE_PERMANENT );
            }
        }
    }
    /*effect eVis = EffectVisualEffect(VFX_FNF_LOS_EVIL_10);
    ApplyEffectToObject(DURATION_TYPE_INSTANT, eVis, oPC);*/
    //FloatingTextStringOnCreature("*uspesne zalozi kristal*", oPC);
...
int SF_GetIsMeleeWeapon(object oItem, object oPC)
{
    ...
    {
        FloatingTextStringOnCreature("Krystal nesedi do teto veci", oPC);
        return FALSE;
    }
    return TRUE;
}
...
int SF_GetIsSocketUpgradable(object oPC, int iBonus, int iSockets)
{
    if (!STACKABLE_GEMS && iBonus)
    {
        FloatingTextStringOnCreature("Kristal tochto typu uz mas v slote", oPC);
        return FALSE;
    }
    if (STACKABLE_GEMS && STACKABLE_GEMS_SHARE_SLOT && !iSockets)
        return TRUE;
    if (!iSockets)
    {
        FloatingTextStringOnCreature("Uz niet viac slotov", oPC);
        return FALSE;
    }
    return TRUE;
}
...
void SF_GetSocketsLeft(object oItem, object oPC)
{
    ...
    {
        FloatingTextStringOnCreature("1 slot volny", oPC);
    }
    else
    {
        FloatingTextStringOnCreature(IntToString(iSockets) + " slotov volnych", oPC);
    }
}
...
void SF_SocketAttackBonusGem(object oItem, object oTarget, object oPC)
{
    ...
    {
        FloatingTextStringOnCreature("Vlozeni tohoto krystalu prekroci " +
...
void SF_SocketKeenGem(object oItem, object oTarget, object oPC)
{
    ...
    else FloatingTextStringOnCreature("Krystal tohoto typu " +
                                        "uz v tomto slotu je", oPC);
}
...
void setSocketedItem(object oItem){
    ...
    IPSafeAddItemProperty( oItem, ItemPropertyCastSpell(IP_CONST_CASTSPELL_UNIQUE_POWER, IP_CONST_CASTSPELL_NUMUSES_UNLIMITED_USE), 0.0f, X2_IP_ADDPROP_POLICY_KEEP_EXISTING );
    SetLocalInt( oItem, "SOCKETS", socketCount );
}

int getIsSocketableItem(object oItem){
